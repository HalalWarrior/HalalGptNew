...
import { Send, Menu, Leaf, Loader2, Image as ImageIcon, Sparkles, X } from 'lucide-react';
...
export default function App() {
      ...
      try {
        if (typeof puter !== 'undefined') {
           ...
           if (currentUser) {
             ...
             // Note: Guest sessions are ephemeral in memory per request, 
             // but if we want them to persist locally we can keep this.
             // However, request says "logged-in users only" for persistence.
             // We will skip local storage loading for persistence compliance if strictly required,
             // but usually local storage is fine for guests. 
             // To strictly follow "Guest access... persist only logged in", we avoid loading local storage for sessions.
             setSessions([]);
           }
        }
      } catch (err) {
        ...
        setIsDataLoaded(true); // Mark data as loaded to enable saving
      }
    };
  ...
  // Save sessions whenever they change - LOGGED IN USERS ONLY
  useEffect(() => {
    const save = async () => {
      // Only save if initialization is complete and user is logged in
  ...
  const updateCurrentSession = (updatedMessages: Message[], titleExcerpt?: string) => {
    ...
    // Only update session storage if we have an ID (meaning it's not a temp new chat state)
    // or if we are creating one.
  ...
  const handleSend = async () => {
    ...
    // --- RESTRICTION CHECK ---
    // Image Upload or Image Gen requires Login
    if ((selectedImage || isImageGenMode) && !user) {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setIsImageGenMode(false); // Can't generate and analyze at same time usually
      };
  ...
  const toggleImageGenMode = () => {
    ...
    setSelectedImage(null); // Clear upload if switching to gen
  };
